...
toad trout turkey turtle weasel whale wolf wombat zebra'''.split()
    words2= '''england spain portugal italy brazil canada japan wales scotland croatia
    egypt sudan senegal iceland denmark argentina mexico cuba honduras australia'''.split()

    infStr = '*\'**\'**\'**\'**\'**\'**\'**\'**\'**\''

    def __init__(self, *args, **kwargs):
        i, j = 2, 0
        self.pics.append(self.hangman[:])
        for ls in self.man.values():
            pic, j = self.hangman[:], 0
            for m in ls:
                pic[i + j] = m
                j += 1
    ...
    def Wordpicker(self):
        ...
        if stamp == 2:
            return self.words2[random.randint(0, len(self.words2) - 1)]
    ...
    def start(self):
        ...
        while True:
            try:
                stamp =int(input("1 Animals 2 Countries:"))
            except:
                print("Not a valid option")
                continue

            word = list(self.Wordpicker())
            ...
            if not success:
                self.info('The word was \'' + ''.join(word) + '\' ! Better Luck next time')
                sys.exit()
